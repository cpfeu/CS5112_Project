...
if __name__ == '__main__':
    ...
    #                                   period=int(100000))
    # bitcoin_dec.decompose_time_series(decompose_with_kalman_filter=False,
    #                                   decompose_with_moving_average=True,
    #                                   show_decomposed_ts=True)
    ...
    # =============== forecasting ===============
    bitcoin_svr = SupportVectorRegression(kernel='poly', degree=5, C=1,
                                          start_idx=0, end_idx=2000,
                                          parser_object=bitcoin_parser,
                                          moving_average_object=bitcoin_ma,
                                          kalman_filter_object=None,
                                          exp_smoothing_object=None)
    bitcoin_svr.train_model()
    bitcoin_svr.test_model()
    google_svr = SupportVectorRegression(kernel='poly', degree=5, C=1,
                                         start_idx=0, end_idx=2000,
                                         parser_object=google_parser,
                                         moving_average_object=google_ma,
                                         kalman_filter_object=None,
                                         exp_smoothing_object=None)
